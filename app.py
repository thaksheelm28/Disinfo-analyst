...
# ---------------------------------------------------------------------------
# /metadata  GET  — required by openenv validate (runtime check)
# ---------------------------------------------------------------------------
...
def metadata() -> JSONResponse:
    ...
    return JSONResponse(content={
        "name": "disinfo-analyst",
        "version": "0.2.0",
        "description": (
            "Disinformation Analyst — three-task OpenEnv environment. "
            "Tasks: node classification (easy), propagation tracing (medium), "
            "claim severity rating (hard)."
        ),
        "tasks": list(TASKS.keys()),
        ...
        "license": "MIT",
        "tags": ["openenv", "content-moderation", "disinformation", "nlp"],
    })


# ---------------------------------------------------------------------------
# /schema  GET  — required by openenv validate (runtime check)
# ---------------------------------------------------------------------------
...
def schema() -> JSONResponse:
    ...
    return JSONResponse(content={
        "action": {
            "type": "union",
            "description": "One of TagAction | TraceAction | ScoreAction | DoneAction",
            "variants": {
                "tag":   {"type": "string", "node_id": "string", "label": "enum[real,disinformation,satire,misleading,unverified]"},
                "trace": {"type": "string", "src": "string", "dst": "string"},
                "score": {"type": "string", "claim_id": "string", "severity": "float[0.0,1.0]"},
                "done":  {"type": "string"},
            },
        },
        "observation": {
        ...
        "state": {
            "type": "object",
            "description": "Same schema as observation — returned by GET /state",
            "fields": {
                "nodes":           "list[NodeObservation]",
                "edges":           "list[EdgeObservation]",
                "claims":          "list[ClaimObservation]",
...
# ---------------------------------------------------------------------------
# /mcp  POST  — required by openenv validate (runtime check)
# ---------------------------------------------------------------------------

@app.post("/mcp", tags=["meta"])
def mcp(request: Dict[str, Any] = None) -> JSONResponse:
    """JSON-RPC 2.0 MCP endpoint (required by openenv validate runtime check)."""
    return JSONResponse(content={
        "jsonrpc": "2.0",
        "id": (request or {}).get("id", None),
        "result": {
            "environment": "disinfo-analyst",
            "version": "0.2.0",
            "capabilities": ["reset", "step", "state", "evaluate"],
        },
    })


# ---------------------------------------------------------------------------
# main() — required by openenv validate (server entry point)
# ---------------------------------------------------------------------------

def main(host: str = "0.0.0.0", port: int = 7860) -> None:
    """Server entry point called by [project.scripts] server = 'app:main'"""
    import uvicorn as _uvicorn
    _uvicorn.run("app:app", host=host, port=port, workers=1)
